import 'package:flutter/material.dart';

class AppColors {
  static const white = Colors.white;
  static const secondary = Color(0xff323335);
  static const darkBlue = Color(0xFF23232D);
  static const lightGray = Color(0xffc0c1c3);
  static const lighterGray = Color(0xFFF3F7FB);
  static const blue_ = Color(0xff2ea8a8);
  static const black = Colors.black;
  static const primary = Color(0xfffe9721);
  static const yellow = Color(0xffe7b40e);
  static const orange = Color(0xfffe9721);
  static const tertiary = Color(0xffff5b36);
}
// additional
// const kDarkPrimaryColor = Color(0xFF212121);
// const kDarkYellow = Color(0xFF373737);
// const kLightPrimaryColor = Color(0xFFFFFFFF);
// const kLightSecondaryColor = Color(0xFFF3F7FB);
// const kYellow = Color(0xFFFFC107);
